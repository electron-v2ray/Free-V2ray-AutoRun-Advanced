...
winreg.SetValueEx(registry_key, "ProxyServer", 0, winreg.REG_SZ, proxy_server + ":" + port_number)
...
configing = config.getint('settings', 'num')


listen_PORT = 4500    # pyprox listening to 127.0.0.1:listen_PORT

num_fragment = configing  # total number of chunks that ClientHello devided into (chunks with random size)

config.read('config.ini')
sleep = config.getfloat('fragment', 'sleep')
fragment_sleep = sleep
...
'cloudflare-dns.com': config.get('settings', 'ip'),
